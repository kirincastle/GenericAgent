...
|- **GA 额外步骤：handoff 生命周期**：GA 的 neat 必须先运行 `memory/handoffs_maintenance.py` 将当前 active 的 handoff 全部 superseded，再写新 handoff 时注册到 `memory/handoffs/handoffs.jsonl`(status:active) 和 `handoffs/index.md`。新会话只看到 1 条 active handoff。
...
> 以下内容为本项目（GenericAgent）专属，覆盖 neat-freak 通用流程未涉及的本项目记忆结构。

### 记忆层级（详见 `memory/memory_management_sop.md`）

```
L1: memory/global_mem_insight.txt   (极简索引, ≤30行)
L2: memory/global_mem.txt           (事实库, 按 ## [SECTION] 组织)
L3: memory/*.md / *.py              (SOP/技能/工具脚本)
L4: memory/L4_raw_sessions/         (历史会话归档)
```

### 跨会话知识提取流程

每次 `/neat` 除了当前对话内容，还应检查 **model_responses/** 中的近期会话：

```
1. ls -t model_responses/ | head -10
2. 从每个文件中提取最后一个 <history>[...]</history> 块（注意 \\n→真换行）
3. 识别涉及的项目/模块/技能
4. 对每个项目，对比 L2 现有条目 → 判断是否有新知识需追加
```
...
| 知识类型 | 写入位置 | 示例 |
|----------|---------|------|
| 环境事实（端口/路径/域名/时区） | L2 `global_mem.txt` 对应 `## [SECTION]` | jpkbb timezone HKT |
| 避坑/教训（高频可复用） | L2 对应 SECTION + 可选 L3 SOP 故障表 | build.sh deploy 后手动 restart |
| 任务流程/SOP 变更 | L3 对应 `*_sop.md` | oc2api_deploy_sop.md 故障排查 |
| 模块/工具新增 | L1 `global_mem_insight.txt` 第二层+L2 新 SECTION | model_registry |
| 场景触发词更新 | L1 括号补充 | console_log_card(v3.027+)→+jpkbb_timezone(hkt) |

### 具体操作步骤

#### Step 1: 盘点当前记忆状态
- `read L2` → `memory/global_mem.txt` 全量读
- `read L1` → `memory/global_mem_insight.txt`
- `ls L3` → 关键 SOP 列表（`memory/*.md`）

#### Step 2: 提取本对话 lesson learned
- 回顾工具调用历史（特别是成功的 code_run / file_patch 操作）
- 提取三类信息：
  - **事实类**：环境配置、版本号、路径等 → L2
  - **教训类**：踩坑、修复方法 → L2 + L3 故障表
  - **流程类**：多步骤操作改进 → L3 SOP

#### Step 3: 写入 L2
- L2 文件只能 `file_patch`（禁止 overwrite）
- 每段以 `## [SECTION] (YYYY-MM-DD)` 开头
- 条目用 `- **` 项目符号
- 如果发现已有同主题 SECTION，合并而非追加新的

#### Step 4: 写入/更新 L3 SOP
- 故障排查表追加新行（`| 问题 | 处理方式 |`）
- 核心流程更新保留旧版做参考，用 ✅/❌ 标记验证状态

#### Step 5: 同步 L1 索引
- 新增 SECTION 加入 L1 第二层
- 已有 SECTION 的版本号/触发词更新（括号内2-4字）
- L1 保持 ≤30 行

### 实际操作示例（本次会话）

```
会话内容: Console Log 时间改为 HKT + jpkbb timezone 切换
↓
L2 更新: `## Console Log Card (v3.027)` + `## jpkbb 服务器 timezone → HKT`
L3 更新: `oc2api_deploy_sop.md` 故障表 + 附加知识
L1 更新: `console_log_card(v3.027+)` + `jpkbb_timezone(hkt)` + `model_registry`
```

### 与通用流程的映射

| neat-freak 通用步骤 | 本项目具体动作 |
|---------------------|--------------|
